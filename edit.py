...
class LineWalker(urwid.ListWalker):
	...
	def combine_focus_with_prev(self):
		...
		self.focus -= 1
		return self.lines[self.focus], self.focus
	...
	def combine_focus_with_next(self):
		...
		return self.lines[self.focus+1], self.focus+1
...
class EditDisplay:
	...
	def __init__(self, name, modi_queue):
		self.save_name = name
	...
	def unhandled_keypress(self, k):
		...
		if k == "f5":
			...
			w, pos = self.walker.combine_focus_with_next()
			self.modi_queue.put(["D", pos, -1])
			print("case Delete")
		elif k == "backspace":
			# backspace at beginning of line
			w, pos = self.walker.combine_focus_with_prev()
			self.modi_queue.put(["D", pos, -1])
			print("case backspace")
		elif k == "enter":
			# start new line
...
def main(name, send_queue):
	try:
		assert open(name, "a")
	except:
		sys.stderr.write(__doc__)
		return
	EditDisplay(name, send_queue).main()


if __name__=="__main__":
	data = core.Data()
	key_str = input("Insert key: ")
	if len(key_str) > 16:
		key_str = key_str[:16]
	elif len(key_str) < 16:
		while len(key_str) == 16:
			key_str = key_str + "0"
	key = core.gen_key(key_str)
	send_queue = Queue()
	main("text.txt", data, key, send_queue)
