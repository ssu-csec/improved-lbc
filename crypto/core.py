import random
import copy
import aes

one_block_len = 14

class Data:
	def __init__(self):
		self.global_meta = []
		self.data = []
	
class Modi_list:
	def __init__(self, index, length):
		self.modi_index = index
		self.modi_length = length
		self.glob_list = []
		self.del_list = []
		self.ins_list = []
	def unpacking(self, data):
		data.global_meta = self.glob_list
		for del_index in self.del_list:
			del data.data[del_index]
		ins_index = self.ins_list.pop(0)
		data.data = data.data[:ins_index] + self.ins_list + data.data[ins_index:]
	def __str__(self):
		return str((self.modi_index, self.modi_length, self.glob_list, self.del_list, self.ins_list))

def gen_key(raw_key):
	ascii_key = aes.str2hex(raw_key)
	key_matrix = aes.block2matrix(ascii_key)
	key = aes.key_schedule_Enc(key_matrix)
	return key

def enc_one(input_str, enc_key, f_link, b_link):		# input_str: int list / enc_key : round key list / f_iv : int type / b_link : int type
	input_block = [f_link]
	input_block.extend(input_str)
	input_block.append(b_link)
	plain_block = aes.block2matrix(input_block)
	output_matrix = aes.AES_128_Encryption(plain_block, enc_key)
	return output_matrix

def dec_one(input_matrix, dec_key, back_link, i):
	out_str = []
	out_matrix = aes.AES_128_Decryption(input_matrix, dec_key)
	for out_row in out_matrix:
		out_str.extend(out_row)
	if back_link != out_str.pop(0) and back_link != 0:
		print("There is something wrong in block number", i)
		out_str[14] = 0
	return out_str

def gen_global(str_len):
	global_str = []
	left_len = str_len
	while left_len > one_block_len:
		global_str.append(one_block_len)
		left_len -= one_block_len
	global_str.append(left_len)
	return global_str

def global_enc(input_str, enc_key):
	block = []
	output_list = []
	front_link = random.randint(0,255)
	back_link = random.randint(0, 255)
	i = 0
	left_len = len(input_str)
	for char in input_str:
		i += 1
		block.append(char)
		left_len -= 1
		if i == one_block_len and left_len != 0:
			enc_block = enc_one(block, enc_key, front_link, back_link)
			output_list.append(enc_block)
			block = []
			i = 0
			front_link = back_link
			back_link = random.randint(0,255)
		elif left_len == 0:
			remain = one_block_len - i
			for j in range(remain):
				block.append(0)
			enc_block = enc_one(block, enc_key, front_link, back_link)
			output_list.append(enc_block)
	return output_list

def global_dec(input_list, dec_key):
	output_str = []
	back_link = 0
	i = 0
	if len(input_list) == 0:
		return output_str
	for input_matrix in input_list:
		dec_str = dec_one(input_matrix, dec_key, back_link, i)
		back_link = dec_str.pop()
		for num in dec_str:
			if num != 0:
				output_str.append(num)
		i += 1
	return output_str

def encrypt(input_str, enc_key, f_iv, b_iv):
	output_list = []
	input_hex = []
	left_len = len(input_str)
	front_link = f_iv
	back_link = random.randint(0,255)
	input_hex = aes.str2hex(input_str)
	i = 0
	block = []
	block_list = []
	for char in input_hex:
		i += 1
		block.append(char)
		left_len -= 1
		if i == one_block_len and left_len != 0:
			enc_block = enc_one(block, enc_key, front_link, back_link)
			block_list.append(enc_block)
			block = []
			i = 0
			front_link = back_link
			back_link = random.randint(0,255)
		elif left_len == 0:
			remain = one_block_len - i
			for j in range(remain):
				block.append(0)
			enc_block = enc_one(block, enc_key, front_link, b_iv)
			block_list.append(enc_block)
			block = []
	return block_list

def decrypt(input_data, dec_key):
	output_str = []
	back_link = 0
	i = 0
	global_str = global_dec(input_data.global_meta, dec_key)
	if len(global_str) == 0:
		print("There is no global_str")
		return output_str
	for enc_matrix in input_data.data:
		if global_str[i] == 0:
			break
		dec_str = dec_one(enc_matrix, dec_key, back_link, i)
		back_link = dec_str.pop()
		for j in range(global_str[i]):
			output_str.append(dec_str[j])
		i += 1

	# test
	output = aes.hex2str(output_str)	
	return output

def search_block_index(global_str, index):
	check = index
	block_index = 0
	if len(global_str) == 0:
		print("There is something wrong!!!!")
		return 0
	while check > 0 and block_index < len(global_str):
		check -= global_str[block_index]
		block_index += 1
	if check < 0:
		block_index -= 1
	elif check > 0:
		pass	
	return block_index

def insert(insert_str, index, input_data, key):
	output_list = Modi_list(index, len(insert_str))		
	global_str = global_dec(input_data.global_meta, key)
	new_glob_str = []
	if index == 0 and len(global_str) == 0:
		in_index = index
		block_index = 0
		f_link = random.randint(0,255)
		b_link = f_link	
	else:
		in_index = index
		block_index = search_block_index(global_str, index)

		if block_index == len(global_str):
			block_index -= 1
			in_index = global_str[block_index]
		else: 
			for i in range(block_index):
				in_index -= global_str[i]
		tmp_matrix = aes.AES_128_Decryption(input_data.data[block_index], key)
		tmp_hex = aes.matrix2block(tmp_matrix)			
		output_list.del_list.append(block_index)							# for networking
		f_link = tmp_hex.pop(0)
		b_link = tmp_hex.pop()
		tmp_str = aes.hex2str(tmp_hex)
		insert_list = list(insert_str)
		insert_str = tmp_str[:in_index] + insert_list + tmp_str[in_index:global_str[block_index]]
		del input_data.data[block_index]
		del global_str[block_index]

	insert_list = encrypt(insert_str, key, f_link, b_link)
	new_global_str = gen_global(len(insert_str))
	global_str = global_str[:block_index] + new_global_str + global_str[block_index:]
	input_data.global_meta = global_enc(global_str, key)
	output_list.glob_list = input_data.global_meta							# for networking
	output_list.ins_list.append(block_index)								# for networking
	output_list.ins_list.extend(insert_list)								# for networking
	input_data.data = input_data.data[:block_index] + insert_list + input_data.data[block_index:]
	
	return output_list

def delete(del_len, index, input_data, key):		# delete length is 1
	output_list = Modi_list(index, -(del_len))
	global_str = global_dec(input_data.global_meta, key)
	index -= 1
	f_block_index = search_block_index(global_str, index)
	f_in_index = index
	
	for i in range(f_block_index):
		f_in_index -= global_str[i]
	
	b_block_index = search_block_index(global_str, index + del_len)
	b_in_index = index + del_len

	for i in range(b_block_index):
		b_in_index -= global_str[i]
	
	if f_in_index == 0 and b_in_index == 0:	 # remove blocks
		if f_block_index == 0:				# remove first block
			if b_block_index == len(global_str):
				global_str = []
				del_list = []
				for i in range(len(global_str) - 1, -1, -1):
					del_list.append(i)
				input_data.data = []
				output_list.del_list.extend(del_list)
			else:
				tmp_block = aes.AES_128_Decryption(input_data.data[f_block_index], key)
				f_link = tmp_block[0][0]
				tmp_block = aes.AES_128_Decryption(input_data.data[b_block_index], key)
				tmp_block[0][0] = f_link
				for i in range(b_block_index, f_block_index - 1, -1):
					del input_data.data[i]
					output_list.del_list.append(i)								# for networking
				tmp_block = aes.AES_128_Encryption(tmp_block, key)
				input_data.data.insert(f_block_index,tmp_block)					# local data modification
				output_list.ins_list.append(tmp_block)							# for networking

		else:
			tmp_block = aes.AES_128_Decryption(input_data.data[b_block_index - 1], key)
			b_link = tmp_block[3][3]
			tmp_block = aes.AES_128_Decryption(input_data.data[f_block_index - 1], key)
			tmp_block[3][3] = b_link
			for i in range(b_block_index - 1, f_block_index - 2, -1):
				del input_data.data[i]
				output_list.del_list.append(i)								# for networking
			tmp_block = aes.AES_128_Encryption(tmp_block, key)
			input_data.data.insert(f_block_index - 1, tmp_block)			# local data modification
			output_list.ins_list.append(tmp_block)
		global_str = global_str[:f_block_index] + global_str[b_block_index:]
		print("Case 1")

	elif f_in_index == 0 and b_in_index != 0:
		tmp_block = aes.AES_128_Decryption(input_data.data[f_block_index], key)
		f_link = tmp_block[0][0]
		tmp_block = aes.AES_128_Decryption(input_data.data[b_block_index], key)
		b_link = tmp_block[3][3]
		tmp_str = aes.matrix2block(tmp_block)
		tmp_str = tmp_str[1:b_in_index + 1]
		tmp_enc = encrypt(aes.hex2str(tmp_str), key, f_link, b_link)
		for i in range(b_block_index, f_block_index - 1, -1):
			del input_data.data[i]                                          # local data modification
			output_list.del_list.append(i)                                  # for networking
		input_data.data = input_data.data[:f_block_index] + tmp_enc + input_data.data[f_block_index:]
		output_list.ins_list.extend(tmp_enc)
		global_str = global_str[:f_block_index] + gen_global(len(tmp_str)) + global_str[b_block_index+1:]
		print("Case 2")

	elif f_in_index != 0 and b_in_index == 0:
		tmp_block = aes.AES_128_Decryption(input_data.data[f_block_index], key)
		f_link = tmp_block[0][0]
		tmp_str = aes.matrix2block(tmp_block)
		tmp_block = aes.AES_128_Decryption(input_data.data[b_block_index - 1], key)
		b_link = tmp_block[3][3]
		tmp_str = tmp_str[1:f_in_index + 1]
		tmp_enc = encrypt(aes.hex2str(tmp_str), key, f_link, b_link)
		for i in range(b_block_index - 1, f_block_index - 1, -1):
			del input_data.data[i]                                          # local data modification
			output_list.del_list.append(i)                                  # for networking
		input_data.data = input_data.data[:f_block_index] + tmp_enc + input_data.data[f_block_index:]
		output_list.ins_list.extend(tmp_enc)
		global_str = global_str[:f_block_index] + gen_global(len(tmp_str)) + global_str[b_block_index:]
		print("Case 3")
	else:
		tmp_block = aes.AES_128_Decryption(input_data.data[f_block_index], key)
		tmp_str = aes.matrix2block(tmp_block)
		
		f_link = tmp_str[0]
		f_tmp_str = tmp_str[1:f_in_index + 1]
		tmp_block = aes.AES_128_Decryption(input_data.data[b_block_index], key)
		tmp_str = aes.matrix2block(tmp_block)
		b_link = tmp_str[15]
		b_tmp_str = tmp_str[b_in_index + 1:15]
		
		tmp_str = f_tmp_str + b_tmp_str
		tmp_enc = encrypt(aes.hex2str(tmp_str), key, f_link, b_link)
		
		for i in range(b_block_index, f_block_index - 1, -1):
			del input_data.data[i]											# local data modification
			output_list.del_list.append(i)                                  # for networking
		input_data.data = input_data.data[:f_block_index] + tmp_enc + input_data.data[f_block_index:]
		output_list.ins_list.extend(tmp_enc)								# for networking
		global_str = global_str[:f_block_index] + gen_global(len(tmp_str)) + global_str[b_block_index + 1:]
		print("Case 4")

	input_data.global_meta = global_enc(global_str, key)					# local global modification
	output_list.glob_list = input_data.global_meta							# for networking

	return output_list 

